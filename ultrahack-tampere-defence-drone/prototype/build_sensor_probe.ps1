...
$App = Join-Path $PSScriptRoot "android_sensor_probe"
...
Invoke-Native @($Apksigner, "sign", "--ks", $Keystore, "--ks-pass", "pass:android", "--key-pass", "pass:android", "--out", (Join-Path $Build "sensor-probe.apk"), (Join-Path $Build "aligned.apk"))
Invoke-Native @($Apksigner, "verify", (Join-Path $Build "sensor-probe.apk"))

Write-Host "Built $Build\sensor-probe.apk"
