...
$App = Join-Path $PSScriptRoot "android_thermovue_xposed_bridge"
...
New-Item -ItemType Directory -Force (Join-Path $Build "stub-classes") | Out-Null
...
$StubSources = Get-ChildItem -Path (Join-Path $App "stubs") -Recurse -Filter *.java | Select-Object -ExpandProperty FullName
Invoke-Native (@("javac", "-source", "11", "-target", "11", "-classpath", $Platform, "-d", (Join-Path $Build "stub-classes")) + @($StubSources))
...
$ClassPath = $Platform + [IO.Path]::PathSeparator + (Join-Path $Build "stub-classes")
Invoke-Native (@("javac", "-source", "11", "-target", "11", "-classpath", $ClassPath, "-d", (Join-Path $Build "classes")) + @($Sources) + @($Generated))
...
$ClassesDex = Join-Path $Build "classes.dex"
$XposedInit = Join-Path $App "assets\xposed_init"
Add-Type -AssemblyName System.IO.Compression.FileSystem
$zip = [IO.Compression.ZipFile]::Open($WithDex, "Update")
try {
    [IO.Compression.ZipFileExtensions]::CreateEntryFromFile($zip, $ClassesDex, "classes.dex") | Out-Null
    [IO.Compression.ZipFileExtensions]::CreateEntryFromFile($zip, $XposedInit, "assets/xposed_init") | Out-Null
} finally {
    $zip.Dispose()
}
...
Invoke-Native @($Zipalign, "-f", "-p", "4", $WithDex, (Join-Path $Build "aligned.apk"))
Invoke-Native @($Apksigner, "sign", "--ks", $Keystore, "--ks-pass", "pass:android", "--key-pass", "pass:android", "--out", (Join-Path $Build "thermovue-xposed-bridge.apk"), (Join-Path $Build "aligned.apk"))
Invoke-Native @($Apksigner, "verify", (Join-Path $Build "thermovue-xposed-bridge.apk"))

Write-Host "Built $Build\thermovue-xposed-bridge.apk"
