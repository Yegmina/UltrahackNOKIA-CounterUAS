...
$App = Join-Path $PSScriptRoot "android_thermovue_instrumentation_probe"
...
New-Item -ItemType Directory -Force (Join-Path $Build "gen") | Out-Null
...
& powershell -NoProfile -Command "Add-Type -AssemblyName System.IO.Compression.FileSystem; `$zip=[IO.Compression.ZipFile]::Open('$Build\with-dex.apk','Update'); [IO.Compression.ZipFileExtensions]::CreateEntryFromFile(`$zip,'$Build\classes.dex','classes.dex') | Out-Null; `$zip.Dispose()"
...
Invoke-Native @($Apksigner, "sign", "--ks", $Keystore, "--ks-pass", "pass:android", "--key-pass", "pass:android", "--out", (Join-Path $Build "thermovue-instrumentation-probe.apk"), (Join-Path $Build "aligned.apk"))
Invoke-Native @($Apksigner, "verify", (Join-Path $Build "thermovue-instrumentation-probe.apk"))

Write-Host "Built $Build\thermovue-instrumentation-probe.apk"
